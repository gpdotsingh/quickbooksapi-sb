...
public class QuickBooksController {
    ...
    /**
     * Legacy Intuit redirect path (/oauth2redirect) forwarded to the main callback.
     * Allows either redirect URI to work without 404.
     */
    @GetMapping("/oauth2redirect")
    public String handleLegacyRedirect(
            @RequestParam("code") String authCode,
            @RequestParam("realmId") String realmId,
            @RequestParam(value = "state", required = false) String state,
            HttpSession session,
            RedirectAttributes redirectAttributes) {
        return handleCallback(authCode, realmId, state, session, redirectAttributes);
    }
